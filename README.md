...
## Быстрый старт с bootstrap.sh

Во всех примерах используется один и тот же репозиторий, но скрипт `bootstrap.sh` запускается локально на нужной ВМ под её роль. 
...
- устанавливает и настраивает окружение CA (easy-rsa, openssl, ufw, каталог `/etc/pki`)
- инициализирует PKI и создаёт Root CA в `/etc/pki/pki`
- выполняет автоматические проверки (наличие `ca.crt`, `ca.key`, права, активный UFW)


### vm-vpn (Этап 2: OpenVPN)
...
- устанавливает OpenVPN и базовую сетевую конфигурацию (IP forwarding, UFW/iptables)
- выводит чеклист для генерации CSR на vm‑vpn, подписи на vm‑ca и раскладки сертификатов
...
2. Клонировать на неё deb‑пакет и репозиторий (команда запускается на локальной машине):
...
4. Запустить автоматизацию через bootstrap (без ручного копирования скриптов):
