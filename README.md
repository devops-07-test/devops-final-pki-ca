...
выполняет автоматические проверки (наличие ca.crt, ca.key, права, UFW)
...
Поднять новую ВМ в Yandex Cloud (Ubuntu 24.04, SSH по ключу).

Скопировать на неё deb‑пакет и репозиторий (команда запускается на локальной машине):

scp artifacts/devops-ca-tools_1.0-1_all.deb user@<ip>:~

scp -r devops-final-pki-ca user@<ip>:~

На ВМ установить зависимости и deb‑пакет:

bash
sudo apt update
sudo apt install -y easy-rsa openssl ufw bash
sudo dpkg -i devops-ca-tools_1.0-1_all.deb
Запустить автоматизацию через bootstrap (без ручного копирования скриптов):

bash
cd ~/devops-final-pki-ca/scripts
chmod +x bootstrap.sh
sudo ./bootstrap.sh stage1
Проверить, что CA создан:
