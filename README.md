...
На текущем этапе реализован первый компонент – CA‑сервер. В дальнейшем будут добавлены:
...
- `install_ca.sh`  
  ...
  - Настраивает UFW: deny incoming, allow outgoing, allow SSH.  
...
Собранный пакет лежит в `artifacts/devops-ca-tools_1.0-1_all.deb`.
...
2. Скопировать на неё deb‑пакет и репозиторий:
